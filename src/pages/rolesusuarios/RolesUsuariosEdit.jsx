...
export default function RolesUsuariosModalEdit({ isOpen, onClose, rolUsuario, onSaved }) {
  const [form, setForm] = useState({ usercod: "", rolescod: "", roleuserest: "AC" });
  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => { if (rolUsuario) setForm(rolUsuario); }, [rolUsuario]);
  ...
  const handleEdit = async () => {
    ...
    setError(null);
    try {
      await rolesUsuariosService.editar(form);
      onSaved();
      onClose();
    } catch (err) {
  ...
  return (
    <Modal isOpen={isOpen}>
      <ModalHeader onClose={onClose}>Editar Rol de Usuario</ModalHeader>
      <ModalBody>
        {error && <Alert variant="error">{error}</Alert>}
        <Input label="Usuario" value={form.usercod} disabled />
        <Input label="Rol" value={form.rolescod} disabled />
        <div className="mt-4">
